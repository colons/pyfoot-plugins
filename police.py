...
def sorted_list_of_crime_types(hood):
    ...
    worst_crime_types = sorted(
        crime_types,
        key=lambda t: t['stats']['crime_rate'], reverse=True)

    return worst_crime_types


def geocode(query):
    url = ('http://maps.googleapis.com/maps/api/geocode/json?address=%s&sensor'
           '=false') % urllib.parse.quote(query)
...
class Plugin(plugin.Plugin):
    """
    CHEESE IT, IT'S THE [ROZZERS](http://police.uk/)
    """
    ...
    def register_commands(self):
        self.commands = [
            ...
            ('crime compare <<location_a>> vs. <<location_b>>',
             self.hood_compare),
        ]
    ...
    def attempt_to_get_one_location(self, message, query):
        ...
        if len(results) == 1:
            ...
            self.irc.privmsg(message.source,
                             self.help_missing % ('location', query))
            return

        elif len(results) > 1:
            self.irc.privmsg(
                message.source,
                ('\x02did you mean\x02 \x03#|\x03 ' +
                 ' \x03#:\x03 '.join([r['formatted_address']
                                      for r in results]))
            )
            return
    ...
    def attempt_to_get_neighbourhood(self, message, latlng, name):
        try:
            ...
            self.irc.privmsg(
                message.source, 'no police found for \x02%s\x02' % name)
            return

        return data

    def hood(self, geocoded):
        crimes = self.query(geocoded['force'], geocoded['neighbourhood'],
                            'crime')['crimes']
    ...
    def hood_summary(self, message, args):
        ...
        >\x02Leicester, UK\x02 \x03#|\x03 crime rate: \x024.53\x02 (average)
        \x03#|\x03 burglary \x03#:\x03 \x020.87\x02 \x03#|\x03 anti social
        behaviour \x03#:\x03 \x020.72\x02 \x03#|\x03 vehicle crime \x03#:\x03
        \x020.58\x02 \x03#|\x03 criminal damage arson \x03#:\x03 \x020.58\x02
        \x03#|\x03 violent crime \x03#:\x03 \x020.43\x02
        """
    ...
    def hood_compare(self, message, args):
        ...
        >\x02Leicester, UK\x02 vs. \x02London, UK\x02 \x03#|\x03 crime rate
        \x03#:\x03 \x0217.61\x02 (above average) vs. \x02207.71\x02 (high)
        \x03#|\x03 commonest crime \x03#:\x03 \x02anti social behaviour\x02
        (3.54) vs. \x02other theft\x02 (92.56) \x03#|\x03 rarest crime
        \x03#:\x03 \x02robbery\x02 (0.00) vs. \x02vehicle crime\x02 (1.33)
        """
        ...
        strings = [
            ...
            % (
                hood_a['api']['all-crime']['crime_rate'],
                hood_a['api']['all-crime']['crime_level'].replace('_', ' '),
                hood_b['api']['all-crime']['crime_rate'],
                hood_b['api']['all-crime']['crime_level'].replace('_', ' '),
            ),

            'commonest crime \x03#:\x03 \x02%s\x02 (%s) vs. \x02%s\x02 (%s)'
            % (
                hood_a['worst'][0]['crime_type'],
                hood_a['worst'][0]['stats']['crime_rate'],
                hood_b['worst'][0]['crime_type'],
                hood_b['worst'][0]['stats']['crime_rate'],
            ),

            # oh god oh god
            'rarest crime \x03#:\x03 \x02%s\x02 (%s) vs. \x02%s\x02 (%s)'
            % (
                hood_a['worst'][-1]['crime_type'],
                hood_a['worst'][-1]['stats']['crime_rate'],
                hood_b['worst'][-1]['crime_type'],
                hood_b['worst'][-1]['stats']['crime_rate'],
            ),
        ]
