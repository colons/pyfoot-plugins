...
class Plugin(plugin.Plugin):
    ...
    def get_quote(self):
        ep_index = self.get_wikitext('A_Show_Episode_Guide').split(
            '=== Notes ===')[0]

        lines = ep_index.split('\n')

        eps = [e.split('[[')[-1].split('|')[0].replace(' ', '_')
               for e in lines
               if len(e) > 0
               and '[[Image:Duckie' in e]
        ...
        epno = int(transcript.split(
            'http://ashow.zefrank.com/episodes/')[1].split(' ')[0])
    ...
    def ze(self, message, args):
        """
        Returns a random quote from the [a show
        transcriptions](http://srcommunity.org/wiki/A_Show_Episode_Guide).
        """
